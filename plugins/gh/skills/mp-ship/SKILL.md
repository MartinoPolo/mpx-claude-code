---
name: ship
description: "Ships finished work end to end by syncing the base, committing, pushing, opening a PR, waiting for CI, then merging. Use when asked to ship or merge completed work; use the dedicated commit, push, or PR skills for individual steps."
...
Invoke the `/mp:sync-base` skill (Skill tool), passing `$ARGUMENTS` as the base branch if provided. It detects the base deterministically via `node ${CLAUDE_PLUGIN_ROOT}/../mp/scripts/detect-base-branch.js`, fetches, merges, resolves conflicts (asks the user on complex ones), and pushes.
...
Fast-path: `node ${CLAUDE_PLUGIN_ROOT}/../mp/scripts/extract-branch-issue.js`
