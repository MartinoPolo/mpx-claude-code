...
You may spawn: `mp-executor`, `mp-checker`, `mp-git-committer`.
...
- Larger or multi-file fix → spawn `mp-executor` with pre-analyzed instructions: per failure, file
  path, root cause, exact change to apply. Never vague "fix the CI".

### 4. Verify locally

If the caller passed local verify commands, spawn `mp-checker` with the ones relevant to the fix.
