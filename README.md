...
**Account tab title** — the pane tint only reads once a pane is focused, so `cc`/`ccw` also name the tab `[P] agentic-setup · yoursafe-components`: account prefix, then the worktree, then the project. Worktree first because it is what differs between panes on one project, and a tab strip truncates from the right; outside a linked worktree the name appears once. `scripts/terminal-title.mts` emits the OSC 0 and takes both prefixes from `statusline-accounts.json`, so an account's tint and its tab label stay in one file. The launcher sets `CLAUDE_CODE_DISABLE_TERMINAL_TITLE=1` alongside it — Claude Code otherwise overwrites the title with the session summary on every status change. The summary is the trade, and only the summary: it lives in memory alone, in no transcript or session file, so nothing can reconstruct it.

**State marker** — disabling that title also took away Claude Code's own `✳`, its tab-strip signal that a session has stopped and wants you. The `UserPromptSubmit` and `Stop` hooks put a state marker back through [`hooks/terminal-title-state.c`](hooks/terminal-title-state.c): a braille spinner (`⠋⠙⠹⠸⠼`, one frame per 100ms) while a turn runs, `🛑` when it stops. Motion rather than a second glyph, because a *static* busy marker sits in the strip looking exactly as attention-worthy as the idle one it has to contrast with — and motion is the distinction Claude Code's own title drew. The idle marker is a free choice once the title is ours, and an octagon rather than the obvious red disc: tab colour already means *project* here, so the marker has to stay legible sitting on a red tab, which a disc or a ring does not. Restyling it was never available — Windows Terminal draws tab titles in the UI font, so the marker is whatever Segoe UI Emoji already draws, with no way to recolour or outline it and no fallback to the profile's terminal font. A `waiting` state on `Notification` was tried and dropped: that event fires more widely than "needs you", and a spurious fire re-marks a tab mid-turn, which is the one failure the marker exists to rule out.

A third event, `SessionStart`, marks `startup`, `resume` and `clear` idle — that is what titles a tab whose session was resumed rather than launched through `cc`/`ccw`. It skips the two sources that fire while a session is working: `compact` cannot tell a manual `/compact`, after which the session really is idle, from an automatic context-limit one, which fires mid-turn with the turn still running, and `fork` cannot tell a `--fork-session` launch from a background `/fork` of a session that is busy this second. Marking a working tab as wanting attention is the failure above. `PreCompact` is unhooked for the mirror-image reason — it would spin the automatic case correctly and leave the manual case with no event to clear it. Closing that last gap needs the helper to mark a busy state as transient, in a file keyed by console pid because a named object dies with the process holding it, so that a `compact` SessionStart clears only the busy states `PreCompact` itself started.

Four traps shape that helper, each of which silently did nothing rather than failing:

| Trap | Effect | Answer |
|---|---|---|
| Claude Code spawns hooks with a console of their own — `GetConsoleWindow() == 0`, one process attached, the executable name as the title | every title write succeeds, against a throwaway console destroyed milliseconds later; the tab never moves. The same probe from a Claude Code *tool* shell reports the terminal's console with `claude` attached, which is what makes a broken version look verified | walk the ancestry to the `claude` process and `AttachConsole` to its console — ConPTY turns that title change into the tab's OSC 0 |
| `FreeConsole`/`AttachConsole` have no .NET wrapper | a PowerShell hook recompiles a C# interop shim on every invocation: 284ms of a 510ms hook, against 151ms of shell startup and 126ms of actual work | a 22KB C helper, ~2ms, and no PowerShell in the chain at all |
| A hook is one-shot — it must return before the turn proceeds | nothing can own a 100ms timer, so a spinner is impossible from the hook itself | `busy` leaves a detached ticker holding a per-console mutex; `idle` signals it and waits for the handover, so the last frame can never overwrite the final title |
| A hook's stdout is a pipe Claude Code reads | on `UserPromptSubmit` an OSC escape would land in the model's prompt rather than the tab | the helper writes the console directly and the hook prints nothing at all |

Rebuild after editing the helper: `gcc -municode -O2 -s -finput-charset=UTF-8 -fwide-exec-charset=UTF-16LE -o hooks/terminal-title-state.exe hooks/terminal-title-state.c`. Verifying it needs a console-attached parent — under `claude -p` from a piped shell no ancestor owns a console, and the helper correctly does nothing.
