...
Run reliable browser verification over a defined scope. This skill owns **scope → surfaces** and orchestration; the reliability rules (sanity-gate, assert-don't-eyeball, programmatic auth, never `networkidle`) live in [`shared/PLAYWRIGHT_TESTING.md`](../shared/PLAYWRIGHT_TESTING.md) and are followed verbatim. $ARGUMENTS
