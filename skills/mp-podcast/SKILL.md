---
name: mp-podcast
description: "Turns a topic plus the listener's own repositories and notes into a two-host educational podcast MP3 in the OneDrive Podcasts folder, generated by NotebookLM with a Gemini multi-speaker TTS fallback."
when_to_use: 'Use when: "podcast about X", "make me a podcast on X".'
...
artifacts and the final MP3.

Resolve machine roots with `env | grep '^MPX_'`. A root that comes back unset is
unavailable — report it and continue without it. When `MPX_PROJECTS`, `MPX_CLONED` and
`MPX_WORK` are all missing, the terminal predates the variables being set; say so, and note
that fixing it takes closing the whole Windows Terminal window.
...
Re-encode to a phone-friendly bitrate and place it in OneDrive:

```bash
ffmpeg -i <downloaded>.mp3 -codec:a libmp3lame -b:a 64k -ac 1 "$MPX_ONEDRIVE/Podcasts/<slug>.mp3"
```
...
- MP3 path as a clickable `file:///` link, plus duration and size
