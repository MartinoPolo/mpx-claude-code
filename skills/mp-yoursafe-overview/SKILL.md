...
description: "Regenerates the self-contained Yoursafe/Verotel HTML onboarding reference from every source plus fresh live snapshots."
...
`$MPX_WORK\Yoursafe\Yoursafe-Onboarding-Overview.html` from the current state of every source.
...
Machine roots (`$MPX_WORK`, `$MPX_OBSIDIAN_VAULT`) are `MPX_*` env vars, resolved at runtime —
`env | grep '^MPX_'` (or PowerShell `$env:MPX_WORK`) — never hardcoded. Markdown does not
interpolate them; resolve the literal value yourself before using a path. If a variable is unset,
ask the user for the path rather than guessing. See `../shared/EXPLORATION.md` § "Paths outside the
working directory".

## Step 1: Load the design contract

Read [REFERENCE.md](REFERENCE.md) and the current
`$MPX_WORK\Yoursafe\Yoursafe-Onboarding-Overview.html`. The regenerated page MUST keep:
...
## Step 2: Re-read static resources (parallel sub-agents)
...
1. **Wiki** — every `.md` in `$MPX_WORK\Yoursafe\wiki\`; note commands, URLs, renaming clues.
2. **Obsidian + Slack** — `$MPX_OBSIDIAN_VAULT\Yoursafe\*.md` and `$MPX_WORK\Yoursafe\team-e-private-summary*.txt`; extract password HINTS, admin facts, project/network renaming.
3. **Repos** — READMEs / `package.json` scripts / `Makefile` targets for `yoursafe-components`, `verotel`, `safe-elements`, `safe-elements-api`, `flowguard-proxy` under `$MPX_WORK\`; note dev/test/deploy commands and staging/prod URLs.
4. **Onboarding scripts** — `$MPX_WORK\netsafe-setup\` and `$MPX_WORK\netsafe-ca\` (quote `apply-nrpt.ps1`, `import-ca.ps1`, handoffs) and `$MPX_WORK\verotel\util\bin\devel-services-win.py`.

## Step 3: Re-mine recent Claude sessions for NEW setup steps (optional)

If setup work continued since the last generation, spawn `general-purpose` sub-agents with `model: "sonnet"` to scan session transcripts in
