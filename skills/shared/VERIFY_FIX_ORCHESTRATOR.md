...
You may spawn sub-agents: `mp-checker`, the `mp-reviewer-*` agents, `mp-executor`, `mp-chrome-devtools-tester`.
...
5. Spawn `mp-executor` with the pre-analyzed fix list — per finding: file path, current code, exact change to apply. Never pass raw findings or vague "fix the issues".
...
3. Spawn `mp-executor` with pre-analyzed fix instructions: file path, root cause, exact change per failure. The executor applies — it does not diagnose.
...
If `browser_verification` is true: spawn `mp-chrome-devtools-tester` for exploratory verification of the changed UI. Fix reported issues via the Phase 1 analyze→mp-executor pattern (counts toward no extra iterations beyond 3 total for this phase). Issues persisting after 3 iterations → append to `unresolved_findings`.
